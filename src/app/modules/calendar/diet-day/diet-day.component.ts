...
export class DietDayComponent implements OnInit, OnChanges {
  ...
  @Output()
  public onConsume: EventEmitter<any> = new EventEmitter<any>();
  ...
  private _productsToUpdate: ConsumedProduct[] = [];
  ...
  public onCompleteChange(event: any) {
    ...
    for (let product of products) {
      ...
      this._productsToUpdate.push(<ConsumedProduct>{
  ...
  public onProductConsume(event: any, product: DietProductItem) {
    ...
    const productToUpdate = this._productsToUpdate.find(
      (x) => x.productId === product.id
    );

    if (productToUpdate) {
      productToUpdate.consumed = event.checked;
    } else {
      this._productsToUpdate.push(<ConsumedProduct>{
